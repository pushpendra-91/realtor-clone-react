    ...
    return (
        <li className=' relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
            <Link className='contents' to={`/category/${listing.type}/${id}`}>
                <img className='h-[170px] w-full hover:scale-105 transition-scale duration-200 ease-in object-cover' loading='lazy' src={listing.imgUrls[0]} alt='' />
                <Moment className='absolute top-2 left-2 bg-[#3377cc] text-white uppercase text-xs font-semibold rounded-md px-1 py-1 shadow-lg
                ...
                <div className='w-full p-[10px]'>
                    ...
                    <div className='flex items-center mt-[10px] space-x-3'>
                        ...
                        <div className='flex items-center space-x-1'>
                            <p className='font-bold text-xs'>{listing.bathrooms > 1 ? `${listing.bathrooms}` : "1 Bath"}</p>
                        </div>
                    </div>
                </div>
            </Link>
